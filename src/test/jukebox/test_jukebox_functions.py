...
class TestJukebox(unittest.TestCase):
    ...
    def setUp(self, MockInitDb):
        ...
        self.session.add(Round(datetime(2019, 5, 1, 0, 0, 0), datetime(2019, 5, 1, 0, 0, 10)))
    ...
    def test_count_votes_current_round(self, MockDatetime):
        ...
        MockDatetime.now.return_value = datetime(2019, 5, 1, 0, 0, 5)
    ...
    @patch('src.jukebox.jukebox_functions.datetime')
    def test_setup_initial_round(self, MockDatetime):
        MockDatetime.now = Mock()
        MockDatetime.now.return_value = datetime(2019, 5, 1, 0, 0, 5)

        round_end = MockDatetime.now.return_value + timedelta(minutes = 0, seconds=2)
        vote_round = Round(MockDatetime.now.return_value, round_end)
        print('vote_round start_date', vote_round.start_date)
        print('vote_round end_date', vote_round.end_date)

        truth = [vote_round.start_date,vote_round.end_date]

        return self.assertEqual(truth, [vote_round.start_date, vote_round.end_date])
